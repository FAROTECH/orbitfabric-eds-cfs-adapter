...
EDSLIB_CONTROL="${P3_EVIDENCE_DIR}/edslib-derived-dispatch-control.txt"
...
if grep -F 'OF_DEMO_APP: payload.enable dispatched through generated EDS interface' "$RUNTIME_LOG" >/dev/null 2>&1; then
  CLASSIFICATION="UNDEFINED_FC_DISPATCHED_TO_VALID_TYPED_HANDLER"
...
CAUSAL_CONTROL="NOT_AVAILABLE"
if [[ -f "$EDSLIB_CONTROL" ]]; then
  if grep -F 'unknown_classification=REJECTED_AT_GENERATED_OF_DEMO_DISPATCH' "$EDSLIB_CONTROL" >/dev/null 2>&1 \
     && grep -F 'nonderived_handler_observed=true' "$EDSLIB_CONTROL" >/dev/null 2>&1; then
    CAUSAL_CONTROL="PASS"
  else
    CAUSAL_CONTROL="INCOMPLETE_OR_FAILED"
  fi
fi

{
  printf '%s\n' '# P3 unknown Function Code postmortem'
  printf '%s\n' 'observation_scope=frozen_pinned_eds_cfs_lane'
  ...
  printf 'edslib_causal_control=%s\n' "$CAUSAL_CONTROL"
...
printf 'P3 EdsLib causal control: %s\n' "$CAUSAL_CONTROL"
