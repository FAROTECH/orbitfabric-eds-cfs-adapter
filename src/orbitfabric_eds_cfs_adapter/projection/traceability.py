...
def _verify_interface(
    ...
    expected_initial_value = _mission_ref_expression(resolved_topic_ref)
    if variable.initial_value != expected_initial_value:
        raise TraceabilityError(
            f"B5 TopicId variable mismatch for {resolved_name}: "
            f"{variable.initial_value!r} != {expected_initial_value!r}"
        )
