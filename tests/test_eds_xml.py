    ...
    return EdsProjectionModel(
        ...
        variables=(
            EdsVariable(
                ...
                read_only=True,
                initial_value="${CFE_MISSION/OF_DEMO_CMD_TOPICID}",
            ),
            EdsVariable(
                name="STATUSTLMTopicId",
                type_ref="BASE_TYPES/uint16",
                read_only=True,
                initial_value="${CFE_MISSION/OF_DEMO_STATUS_TLM_TOPICID}",
            ),
        ),
        parameter_maps=(
            EdsParameterMap(
...
def test_b5_concepts_are_preserved_in_xml() -> None:
    ...
    assert [(item.attrib["name"], item.attrib["initialValue"]) for item in variables] == [
        ("CMDTopicId", "${CFE_MISSION/OF_DEMO_CMD_TOPICID}"),
        ("STATUSTLMTopicId", "${CFE_MISSION/OF_DEMO_STATUS_TLM_TOPICID}"),
    ]
