...
def canonical_identity(root: Path) -> dict[str, Any]:
    constants_path = root / "src" / "orbitfabric_eds_cfs_adapter" / "constants.py"
    ...
    normalized_coordinate: dict[str, str] = {}
    for key in ("authority", "publisher", "name"):
        value = coordinate.get(key)
        if not isinstance(value, str) or not value.strip():
            raise ValueError(f"SOURCE_COORDINATE.{key} must be a non-empty string")
        normalized_coordinate[key] = value.strip()

    version = payload.get("VERSION")
    if not isinstance(version, str) or not version.strip():
        raise ValueError("VERSION must be a non-empty string")

    return {"source_coordinate": normalized_coordinate, "version": version.strip()}
...
def build_release_descriptor(
    *,
    wheel: Path,
    manifest: Path,
    source_coordinate: dict[str, str],
    release_version: str,
    artifact_id: str,
    artifact_type: str,
) -> dict[str, Any]:
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description=(
            "Build the canonical OrbitFabric EDS-cFS Adapter Release Descriptor and, unless "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if version.strip() != identity["version"]:
        raise SystemExit(
            "Release version differs from canonical adapter identity: "
            f"{version!r} != {identity['version']!r}"
        )
